...
#################################################################################################
#################################################################################################
####### ANALYSE AVEC LES DONNEES DE HIVE DIRECTEMENT EXPORTÉES DEPUIS LES TABLES HIVE ###########
#################################################################################################
#################################################################################################
...
# Importer les données de Catalogue 
CData <- rhive.query("SELECT * FROM projetBigData.Catalogue")
...
#####################################################################################
###### 3 du PDF : Application des classes / clusters à Immatriculations #############
#####################################################################################
...
ImmData <- rhive.query("SELECT * FROM projetBigData.Immatriculations_ext")
...
# Encoere une fois ici, on ne peut pas définir de nom aux cluster sans intervention humaine du fait 
...
# Charger les données
Marketing <- rhive.query("SELECT * FROM projetBigData.Marketing_ext")

# Nommer les colonnes
colnames(Marketing) <- c("age", "sexe", "taux", "situationFamiliale", "nbEnfantsAcharge", "SecondeVoiture")
...
write.csv(Marketing, file = "C:/vagrant-projects/OracleDatabase/21.3.0/ProjetTPABigData/R/Marketing_cluster.csv", row.names = TRUE)
